...
def generate_ffmpeg_commands(video_file, time_ranges, output_prefix, gpu, preview=False, overwrite=False, args=None):
    ...
    for srt_id, start, end in time_ranges:
        ...
        if preview:
            ...
            preview_text_1 = escape_text(f"ID: {srt_id}, Start: {seconds_to_srt_timestamp(start)}, End: {seconds_to_srt_timestamp(end)}")
            ...
            # preview text 2
            preview_text_2 = escape_text(f"SRT file: {os.path.basename(args.srt_file_path)}")
